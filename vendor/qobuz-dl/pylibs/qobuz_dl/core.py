...
class QobuzDL:
    ...
    def initialize_client(self, app_id, secrets, private_key, token_file):
        self.client = qopy.Client(app_id, secrets, private_key, token_file)
    ...
    def download_from_txt_file(self, txt_file):
        with open(txt_file, "r") as txt:
            try:
                ...
                return
            logger.info(
                f"{YELLOW}qobuz-dlp will download {len(urls)}"
    ...
    def lucky_mode(self, query, download=True):
        ...
        logger.info(
            ...
            f"{YELLOW}qobuz-dlp will attempt to download the first "
